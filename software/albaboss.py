...
if __name__=='__main__':
    ...
    for cmd in args.init:
        ...
        if retval!=0:
            ...
            if('config_fpga' in cmd):
                exit(1) #exit if config fpga failed, making albaboss restart.
        else:
    ...
    for cmd in args.commands:  #loop over commands, and start a thread for each that will spawn subprocesses
        ...
        x.start()
        xx.add(x)
    for x in xx:  #albaboss will exit only when both threads finish. .join() waits for all spawned threads to exit.
        x.join()  #Further, a thread will exit only when the subprocess it spawned exits.
